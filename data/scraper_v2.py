...
class Scraper:
    ...
    def build_story_tree(self, url):
        ...
        self.G.add_node(self.event_num, text=text)
    ...
    def build_tree_helper(self, parent_story, action_num, depth, prev_event, old_actions):
        ...
        if result == parent_story or result in self.texts:
            ...
            self.G.add_edge(prev_event, self.event_num)
            return None

        self.events.append(result)
        self.event_num = len(self.events) - 1
        self.G.add_node(self.event_num, text=result)
        self.G.add_edge(prev_event, self.event_num)
...
def save_tree(tree, filename):
    ...
    with open(os.path.join(curr_dir, filename), "w+") as fp:
        json.dump(tree, fp)
